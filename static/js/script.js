...
/* -------------------- DURATIONS -------------------- */
async function initDurations() {
  await fetch("/api/init-durations", { method: "POST" });
}
...
async function getPlaylistDuration(playlistName) {
    ...
    const res = await fetch("/api/get-durations", {
        method: "POST",
...
function formatPlaylistDuration(durationsObject) {
    let totalDuration = 0;
    for (const value of Object.values(durationsObject)) {
        if (typeof value === "number") totalDuration += value;
    }
...
async function setDurationsForPlaylists() {
    ...
    const promises = playlistCards.map(async (el) => {
        ...
        const durations = await getPlaylistDuration(playlistName);
        if (p) p.textContent = formatPlaylistDuration(durations);
    });
        ...
        el.addEventListener('click', (event) => {
            ...
            for (const e of songsListDiv.children) {
                if (currentSong && e.dataset.songName === playbarSongName.textContent) {
                    toggleSongPlaying(e.dataset.songId, true);
                }
            }
...
function toggleSongPlaying(songId, songsListChanged = false) {
    ...
    if (!songsListChanged) {
        if (clickedSongCard.dataset.songPlaying === "false") {
            ...
            if (currentPlaylistId) {
                for (const sibling of playlistListDiv.children) {
                    sibling.dataset.playlistSelected = "false";
                    sibling.querySelector('img.play-btn').src = getPlayPauseSVGs('play');
                }
                const plCard = document.querySelector(`[data-playlist-id="${currentPlaylistId}"]`)
                plCard.dataset.playlistSelected = "true"
                plCard.querySelector('img.play-btn').src = getPlayPauseSVGs('pause');
            }
        }
        else {
...
async function loadSongsData() {
  ...
  songsLinks = data.songsLinks;
  songsByPlaylist = data.songsByPlaylist;
}
